...
# -------------------------
df = inter.merge(users, on="user_id", how="left").merge(plans, on="plan_id", how="left")
...
income_fallback = float(np.nanmedian(income_safe)) if np.isfinite(np.nanmedian(income_safe)) else 1.0
df["premium_income_ratio"] = df["premium"] / income_safe.fillna(income_fallback)
...
df["cond_count"] = df.get("chronic_conditions", "none").fillna("none").apply(
    lambda s: 0 if s in ["", "none"] else len([x for x in str(s).split(";") if x.strip()])
)

# Preference match: provider inside preferred_providers (row-wise, null-safe)
def provider_match_row(r):
    prefs = [p.strip().lower() for p in str(r.get("preferred_providers", "")).split(";") if p.strip()]
...
if len(valid_df):
    ...
    mini_out = mini[["session_id","user_id","plan_id","score","shown_rank","clicked","chosen"]]
